...
LRESULT CALLBACK DemoApp::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		if (pDemoApp)
		{
			...
			case WM_LBUTTONUP:
			{
				...
				if (isPistol || isRifle || isShotgun)
				{
					...
					if (currentGun == selectingGun) {
						if (isMouseIn(currentMousePosition, targetRectangle)) score += 100;
						else if (isMouseIn(currentMousePosition, trapRectangle)) score -= 50;	// 총은 맞으나 트랩 명중
						else if (isMouseIn(currentMousePosition, deerRectangle) || isMouseIn(currentMousePosition, rabbitRectangle)) score -= 150;  // 동물 명중
					}
					else if (currentGun != selectingGun) {
						if (isMouseIn(currentMousePosition, targetRectangle)) score += 30;
						else if (isMouseIn(currentMousePosition, trapRectangle)) score -= 80;	// 트랩 명중
						else if (isMouseIn(currentMousePosition, deerRectangle) || isMouseIn(currentMousePosition, rabbitRectangle)) score -= 150;  // 동물 명중
					}
					else score += 0;


					if (isMouseIn(currentMousePosition, targetRectangle) || isMouseIn(currentMousePosition, trapRectangle))
						pDemoApp->soundOn(pDemoApp, targetSound, FALSE);
					else if (isMouseIn(currentMousePosition, deerRectangle) || isMouseIn(currentMousePosition, rabbitRectangle))
						pDemoApp->soundOn(pDemoApp, animalSound, FALSE);

				}

				if (isPistol)
					pDemoApp->soundOn(pDemoApp, pistolSound, FALSE);
				else if (isRifle)
					pDemoApp->soundOn(pDemoApp, rifleSound, FALSE);
				else if (isShotgun)
					pDemoApp->soundOn(pDemoApp, shotgunSound, FALSE);
			...
			case WM_LBUTTONDOWN:
			{
				if (isMouseIn(currentMousePosition, gunRectangle[0])) {
					isPistol = true;
					pDemoApp->soundOn(pDemoApp, reloadSound, FALSE);
				}
				else if (isMouseIn(currentMousePosition, gunRectangle[1])) {
					isRifle = true;
					pDemoApp->soundOn(pDemoApp, reloadSound, FALSE);
				}
				else if (isMouseIn(currentMousePosition, gunRectangle[2])) {
					isShotgun = true;
					pDemoApp->soundOn(pDemoApp, reloadSound, FALSE);
				}
				InvalidateRect(hwnd, NULL, FALSE);
			...
			case WM_PAINT:
			{
				...
				if (time(NULL) - targetTime > 10)	// 사격 제한 시간(8초 이내에 사격하지 않을 경우 -20점)
				{
					score -= 20;
					currentGun = init;
					selectingGun = init;
			...
			case WM_KEYDOWN:
			{
				...
				case VK_RIGHT: {
					if (volume != 100)
						volume = volume + 5;
					pDemoApp->SetVolume(pDemoApp->soundManager->pDSBPrimary, volume);
					break;
				}
				case VK_LEFT: {
					if (volume != 0)
